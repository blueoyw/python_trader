...
class Crawl(threading.Thread):
    def __init__(self, secs):
        ...
        self.fee = 1.2 # cryptocurrency market fee
        ...
        self.status_tick = 2 # monitoring count tick
    ...
    '''
    def logic( self, currency, signal, result ):
        if self.status == 'monitoring_buy':
            if result[currency]['last'] > result[currency]['sma_15']:
                self.status = 'buy'
    '''
    ...
    def getSignal( self, date, result ):
        ...
        #to do buy or sell or hold
        #volume_rate is high and down_cross sell
        #-2% loss sell
        LOG.info("end getSignal")
    ...
    def getPrice(self, currency ):
        ...
        #ywoh to do raise exception and register chk_proess.sh on crontab
        if result is None :
        ...
        # lambda test
        #df['date'] = df['timestamp'].apply( test )               
        #df = df.set_index( 'date' ) # index 변경        
        LOG.debug( df.to_string() )        
